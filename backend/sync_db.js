...
const MOCK_BUILDINGS = [
  {
    ...
    rating: "4.0",
    image: "https://images.unsplash.com/photo-1759837107238-7637c2446e6c?w=600&h=400&fit=crop",
  ...
  },
  {
    ...
    rating: "4.8",
    image: "https://images.unsplash.com/photo-1697399939714-b5adbb642c12?w=600&h=400&fit=crop",
...
async function sync() {
    ...
    // First, let's get all existing buildings
    const { data: existingData, error: selErr } = await supabase.from('buildings').select('*');
    if (selErr) console.error("Select error:", selErr);
    const existing = existingData || [];
    
    for (let b of MOCK_BUILDINGS) {
        // try to find by name, or partial match for Mnazi Mmoja
        const found = existing.find(eb => 
            eb.name === b.name || 
            (eb.name.includes("Mnazi Mmoja") && b.name.includes("Mnazi Mmoja")) ||
            (eb.name.includes("St. Joseph") && b.name.includes("St. Joseph"))
        );
        
        if (found) {
            console.log("Updating:", found.name, "to have image:", b.image);
            ...
            console.log("Inserting new building:", b.name);
